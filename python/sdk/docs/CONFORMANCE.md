...
- **`abortedAt` with parallel validators**: the SEP field is singular; when several validators fail in parallel this SDK reports the first blocking failure in chain-entry order (all results still appear in `results` and the summary). A chain-level `timeoutMs` that fires during a parallel stage (validators or sinks) has no single in-flight entry, so `abortedAt.interceptor` is `""` and the stage is named in `abortedAt.reason` instead; a timeout during the sequential mutation stage names the specific mutator.
- **Per-entry `timeoutMs` breach maps to the stage's status, not `timeout`**: the `"timeout"` chain status is reserved for the chain-level aggregate timeout. A fail-closed per-interceptor timeout is treated as that interceptor failing, so it aborts as `validation_failed` / `mutation_failed` (with the underlying `TimeoutError` in `abortedAt.reason`), matching how any other interceptor error is routed by `failOpen`.
...
- **`-32603` named after mutations but used generically**: the SEP labels `-32603` "Interceptor mutation failed", yet it is the natural catch-all when *any* interceptor's handler crashes or returns the wrong result type. This SDK keeps the SEP-aligned constant name (`INTERCEPTOR_MUTATION_FAILED`) and uses it for every interceptor type; the SEP could rename the assignment to "interceptor execution failed" to match how implementations use it.
- **Chain-level deadline is not communicated to servers**: `ChainExecutionParams.timeout_ms` is a deadline over the whole chain, enforced invoker-side; the per-invoke `timeoutMs` field is only sent when an `InterceptorOverrides.timeout_ms` sets it, so a server may keep executing after the invoker has already timed out. Propagating the deadline would mean sending each invoke the *remaining* budget (the full budget would overstate it for later entries), and a server-side breach of that value surfaces as that interceptor failing (routed through `failOpen` like any other error), not as the chain's `timeout` status. None of the SDKs in this repo propagate it today; the SEP could say whether `timeoutMs` is per-call only or also the vehicle for caller deadlines.

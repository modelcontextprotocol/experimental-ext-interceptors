...
/// <summary>
/// Provides extension methods for creating MCP server interceptors directly (without DI builder).
/// </summary>
public static class McpServerInterceptorExtensions
{
    ...
    /// <summary>
    /// Creates <see cref="McpServerInterceptor"/> instances from a type.
    /// </summary>
    ...
    /// <returns>A collection of <see cref="McpServerInterceptor"/> instances.</returns>
    /// <remarks>
    ...
    /// type, where the methods are attributed as <see cref="McpServerInterceptorAttribute"/>, and creates an <see cref="McpServerInterceptor"/>
    /// instance for each.
    /// </remarks>
    public static IEnumerable<McpServerInterceptor> WithInterceptors<[DynamicallyAccessedMembers(
        ...
        foreach (var interceptorMethod in typeof(TInterceptorType).GetMethods(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.Instance))
        {
            if (interceptorMethod.GetCustomAttribute<McpServerInterceptorAttribute>() is not null)
            {
                yield return interceptorMethod.IsStatic
                    ? McpServerInterceptor.Create(interceptorMethod, options: new() { Services = services, SerializerOptions = serializerOptions })
                    : McpServerInterceptor.Create(interceptorMethod, ctx => CreateTarget(ctx.Services, typeof(TInterceptorType)), new() { Services = services, SerializerOptions = serializerOptions });
            }
        }
    }

    /// <summary>
    /// Creates <see cref="McpServerInterceptor"/> instances from a target instance.
    /// </summary>
    /// <typeparam name="TInterceptorType">The interceptor type.</typeparam>
    ...
    /// </para>
    /// <para>
    /// If <typeparamref name="TInterceptorType"/> is itself an <see cref="IEnumerable{T}"/> of <see cref="McpServerInterceptor"/>,
    ...
    private static IEnumerable<McpServerInterceptor> GetInterceptorsFromTarget<[DynamicallyAccessedMembers(
        DynamicallyAccessedMemberTypes.PublicMethods |
        DynamicallyAccessedMemberTypes.NonPublicMethods)] TInterceptorType>(
        TInterceptorType target,
        JsonSerializerOptions? serializerOptions)
    {
        foreach (var interceptorMethod in typeof(TInterceptorType).GetMethods(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.Instance))
        {
            if (interceptorMethod.GetCustomAttribute<McpServerInterceptorAttribute>() is not null)
            {
                yield return McpServerInterceptor.Create(
                    interceptorMethod,
    ...
    /// <summary>
    /// Creates <see cref="McpServerInterceptor"/> instances from types.
    /// </summary>
    /// <param name="interceptorTypes">Types with <see cref="McpServerInterceptorAttribute"/>-attributed methods to add as interceptors.</param>
    /// <param name="services">Optional service provider for dependency injection.</param>
    /// <param name="serializerOptions">The serializer options governing interceptor parameter marshalling.</param>
    /// <returns>A collection of <see cref="McpServerInterceptor"/> instances.</returns>
    ...
    /// types, where the methods are attributed as <see cref="McpServerInterceptorAttribute"/>, and creates an <see cref="McpServerInterceptor"/>
    /// instance for each.
    /// </remarks>
    [RequiresUnreferencedCode(WithInterceptorsRequiresUnreferencedCodeMessage)]
    public static IEnumerable<McpServerInterceptor> WithInterceptors(
        ...
        foreach (var interceptorType in interceptorTypes)
        {
            ...
            foreach (var interceptorMethod in interceptorType.GetMethods(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.Instance))
            {
                if (interceptorMethod.GetCustomAttribute<McpServerInterceptorAttribute>() is not null)
                {
                    yield return interceptorMethod.IsStatic
                        ? McpServerInterceptor.Create(interceptorMethod, options: new() { Services = services, SerializerOptions = serializerOptions })
                        : McpServerInterceptor.Create(interceptorMethod, ctx => CreateTarget(ctx.Services, interceptorType), new() { Services = services, SerializerOptions = serializerOptions });
                }
            }
        }
    }
    ...
    /// <summary>
    /// Creates <see cref="McpServerInterceptor"/> instances from types marked with <see cref="McpServerInterceptorTypeAttribute"/> in an assembly.
    /// </summary>
    ...
    /// classes that are marked with the <see cref="McpServerInterceptorAttribute"/> and creates <see cref="McpServerInterceptor"/>s.
    /// </para>
    /// <para>
    ...
    /// </remarks>
    [RequiresUnreferencedCode(WithInterceptorsRequiresUnreferencedCodeMessage)]
    public static IEnumerable<McpServerInterceptor> WithInterceptorsFromAssembly(
